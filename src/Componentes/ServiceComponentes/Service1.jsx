...
export default function Service1() {
  ...
  return (
    <div className="w-[100vw] overflow-x-hidden">
      {data &&
        data.map((item, index) => (
          <div className={`container py-6  m-auto  flex flex-wrap items-center  justify-center ${(index+2)%2===0 ? "" : "flex-row-reverse" }`}  >
            ...
            <div className={`w-full p-2 lg:p-0 lg:w-[48%] flex  ${(index+2)%2===0 ?  "lg:justify-end":"lg:justify-start"}`} >
              <img alt="imagd" src={item.image} className="w-[95%] lg:w-[80%] rounded-[20px]  lg:rounded-[50px]" />
            </div>
          </div>
        ))}
    </div>
