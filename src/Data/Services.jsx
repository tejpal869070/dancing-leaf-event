export const PrivateServices = {
  heading: "Unforgettable Private Parties: Making Your Moments Extraordinary",
  name: "Private Events",
  mainimg: require("../assets/rent-9.jpg"),
  detail:
    "We specialize in transforming private parties like birthdays, baby showers, and housewarmings into unforgettable experiences. Our dedicated team meticulously plans and flawlessly executes every detail, ensuring your vision comes to life with creativity and precision. From initial concept to final cleanup, we make your event stress-free and exceptional.",
  services: [
    {
      id: 2,
      heading: "Birthday Party",
      image: require("../assets/birthday-2.jpg"),
      desc: "Elevate any birthday celebration with our personalized decorations and delectable custom cakes! Whether you're turning one or one hundred, our themed balloons and party favors will make your special day truly memorable.",
    },
    {
      id: 1,
      heading: "Baby shower",
      image: require("../assets/baby-showe.jpg"),
      desc: "Get ready to welcome your bundle of joy with our charming custom cakes and delightful decorations tailored just for baby showers! Let us make your celebration extra special with our adorable themes and party favors. ",
    },
    {
      id: 3,
      heading: "Gender Reveal",
      image: require("../assets/gender-reveal.jpg"),
      desc: "Make your gender reveal party an unforgettable event! Let us create a celebration filled with excitement and anticipation as you announce the newest addition to your family. With personalized touches and vibrant décor, it's a moment you'll cherish forever.",
    },
    {
      id: 4,
      heading: "House warming",
      image: require("../assets/house-warmimg.jpg"),
      desc: "Welcome to your new home in style! Our housewarming events are designed to celebrate this exciting milestone with warmth and elegance. From inviting décor to delicious catering, let us create an unforgettable gathering as you embark on this new chapter of your life.",
    },
    {
      id: 4,
      heading: "Theme Picnic",
      image: require("../assets/themeparty.jpg"),
      desc: "Step into a world of imagination with our themed parties! From elegant masquerades to lively tropical fiestas, we bring your vision to life with creative décor, entertainment, and cuisine. Let us transform your event into an unforgettable experience filled with excitement and wonder.",
    },
    {
      id: 6,
      heading: "Midnight Surprise",
      image: require("../assets/midnight.jpg"),
      desc: "Add a dash of mystery and excitement to your celebrations with our Midnight Surprise events! Whether it's a birthday, anniversary, or special occasion, we specialize in orchestrating unforgettable surprises that unfold under the enchanting veil of the night sky.",
    },
    {
      id: 7,
      heading: "Themed Events",
      image: require("../assets/themeparty2.jpg"),
      desc: "Embark on a journey of creativity and imagination with our themed events! From elegant galas to whimsical parties, we specialize in bringing your vision to life. With meticulous attention to detail and immersive decor.",
    },
    {
      id: 7,
      heading: "Graduation Party",
      image: require("../assets/graduation.jpg"),
      desc: "Embark on a journey of creativity and imagination with our themed events! From elegant galas to whimsical parties, we specialize in bringing your vision to life. With meticulous attention to detail and immersive decor.",
    },
    {
      id: 7,
      heading: "Holiday Celebration",
      image: require("../assets/holiday.jpg"),
      desc: "Embark on a journey of creativity and imagination with our themed events! From elegant galas to whimsical parties, we specialize in bringing your vision to life. With meticulous attention to detail and immersive decor.",
    },
    {
      id: 7,
      heading: "Retirement Party",
      image: require("../assets/retire.jpg"),
      desc: "Embark on a journey of creativity and imagination with our themed events! From elegant galas to whimsical parties, we specialize in bringing your vision to life. With meticulous attention to detail and immersive decor.",
    },
  ],
};

export const CorporateServices = {
  heading: "Professional Corporate Event Planning: Elevate Your Business Gatherings",
  name: "Corporte Events",
  mainimg: require("../assets/corporate-2.jpg"),
  detail:
    "We specialize in creating exceptional corporate parties and events that leave a lasting impression. Whether it's a product launch, team building retreat, or corporate anniversary, our meticulous planning and flawless execution ensure every detail is perfect. From venue selection to decor and entertainment, we bring your vision to life with creativity and professionalism, guaranteeing a memorable experience for all attendees.",
  services: [
    {
      id: 2,
      heading: "Company Anniversary",
      image: require("../assets/companyanniver.jpg"),
      desc: "Elevate any birthday celebration with our personalized decorations and delectable custom cakes! Whether you're turning one or one hundred, our themed balloons and party favors will make your special day truly memorable.",
    }, 
    {
      id: 3,
      heading: "Award Ceremony",
      image: require("../assets/award.png"),
      desc: "Make your gender reveal party an unforgettable event! Let us create a celebration filled with excitement and anticipation as you announce the newest addition to your family. With personalized touches and vibrant décor, it's a moment you'll cherish forever.",
    },
    {
      id: 4,
      heading: "Team-Building Events",
      image: require("../assets/teambuilding.jpg"),
      desc: "Welcome to your new home in style! Our housewarming events are designed to celebrate this exciting milestone with warmth and elegance. From inviting décor to delicious catering, let us create an unforgettable gathering as you embark on this new chapter of your life.",
    },
    {
      id: 4,
      heading: "Client Appreciation",
      image: require("../assets/clintapp.jpg"),
      desc: "Step into a world of imagination with our themed parties! From elegant masquerades to lively tropical fiestas, we bring your vision to life with creative décor, entertainment, and cuisine. Let us transform your event into an unforgettable experience filled with excitement and wonder.",
    },
    {
      id: 5,
      heading: "Grand Opening",
      image: require("../assets/grandop.jpg"),
      desc: "Elevate your special day with our wedding party services! From intimate gatherings to grand celebrations, we tailor every detail to reflect your love story. With exquisite décor, delicious cuisine, and flawless coordination, let us create the wedding of your dreams, filled with joy and cherished memories.",
    },
    {
      id: 6,
      heading: "Product Launch",
      image: require("../assets/prodlumch.png"),
      desc: "Add a dash of mystery and excitement to your celebrations with our Midnight Surprise events! Whether it's a birthday, anniversary, or special occasion, we specialize in orchestrating unforgettable surprises that unfold under the enchanting veil of the night sky.",
    },
    {
      id: 7,
      heading: "Networking events",
      image: require("../assets/networkinev.jpg"),
      desc: "Embark on a journey of creativity and imagination with our themed events! From elegant galas to whimsical parties, we specialize in bringing your vision to life. With meticulous attention to detail and immersive decor.",
    },
    {
      id: 7,
      heading: "Corporte Retreats",
      image: require("../assets/corporetres.jpg"),
      desc: "Embark on a journey of creativity and imagination with our themed events! From elegant galas to whimsical parties, we specialize in bringing your vision to life. With meticulous attention to detail and immersive decor.",
    },
  ],
};

export const WeddingServices = {
  heading:
    "Exquisite Wedding Events: Where Dreams Become Unforgettable Memories",
  name: "Wedding Parties",
  mainimg: require("../assets/wedding.jpg"),
  detail:
    "We specialize in creating unforgettable wedding events tailored to your vision. Whether it's an intimate ceremony or a lavish celebration, our team meticulously plans every detail to perfection. From venue selection and elegant décor to delicious catering and lively entertainment, we ensure your special day is stress-free and magical. Let us turn your wedding dreams into a reality, crafting cherished memories that will be treasured forever.",
  services: [
    {
      id: 2,
      heading: "Wedding Day",
      image: require("../assets/marriage-day.jpg"),
      desc: "Elevate any birthday celebration with our personalized decorations and delectable custom cakes! Whether you're turning one or one hundred, our themed balloons and party favors will make your special day truly memorable.",
    },
    {
      id: 1,
      heading: "Marriage Proposal",
      image: require("../assets/porpose.jpg"),
      desc: "Get ready to welcome your bundle of joy with our charming custom cakes and delightful decorations tailored just for baby showers! Let us make your celebration extra special with our adorable themes and party favors. ",
    },
    {
      id: 3,
      heading: "Engagement Party",
      image: require("../assets/engeg.jpg"),
      desc: "Make your gender reveal party an unforgettable event! Let us create a celebration filled with excitement and anticipation as you announce the newest addition to your family. With personalized touches and vibrant décor, it's a moment you'll cherish forever.",
    },
    {
      id: 4,
      heading: "Destination Wedding",
      image: require("../assets/destwedding.jpeg"),
      desc: "Welcome to your new home in style! Our housewarming events are designed to celebrate this exciting milestone with warmth and elegance. From inviting décor to delicious catering, let us create an unforgettable gathering as you embark on this new chapter of your life.",
    },
    {
      id: 4,
      heading: "Wedding Anniversary",
      image: require("../assets/annivercy.jpg"),
      desc: "Step into a world of imagination with our themed parties! From elegant masquerades to lively tropical fiestas, we bring your vision to life with creative décor, entertainment, and cuisine. Let us transform your event into an unforgettable experience filled with excitement and wonder.",
    },
    {
      id: 4,
      heading: "Bridal Shower",
      image: require("../assets/brideshow.webp"),
      desc: "Step into a world of imagination with our themed parties! From elegant masquerades to lively tropical fiestas, we bring your vision to life with creative décor, entertainment, and cuisine. Let us transform your event into an unforgettable experience filled with excitement and wonder.",
    },
  ],
};
